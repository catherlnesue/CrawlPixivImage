...
class CrawlerPixivImg(object):
    ...
    def DownloadProcess(self, url, IMGLibrary = {}):
        ...
        status = requests.get(url, headers=headers, timeout=1).status_code
        chunk_size = 1024
        ...
        if status != (404 or 403):
            ...
            imgsize = requests.get(url, headers=headers, timeout=1).headers['content-length']
            ...
            with requests.get(url, headers=headers, timeout=1, stream=True) as img:
